...
import 'package:myservice_customer/component/base_scaffold_body.dart';
import 'package:myservice_customer/main.dart';
import 'package:myservice_customer/network/rest_apis.dart';
import 'package:myservice_customer/screens/auth/auth_user_services.dart';
import 'package:myservice_customer/screens/auth/forgot_password_screen.dart';
import 'package:myservice_customer/screens/auth/otp_login_screen.dart';
import 'package:myservice_customer/screens/auth/sign_up_screen.dart';
...
class _SignInScreenState extends State<SignInScreen> {
  ...
  void _handleLoginUsers() async {
    ...
    await loginCurrentUsers(context, req: request).then((value) async {
      ...
      saveDataToPreference(context, userData: value.userData!, onRedirectionClick: () {
  ...
  void googleSignIn() async {
    ...
    await authService.signInWithGoogle(context).then((value) async {
      appStore.setLoading(false);
      saveDataToPreference(context, userData: value!.userData!, isSocialLogin: true, onRedirectionClick: () {
        onLoginSuccessRedirection();
      });
    }).catchError((e) {
      appStore.setLoading(false);
  ...
  Widget _buildSocialWidget() {
    if (otherSettingStore.socialLoginEnable.getBoolInt()) {
      return Column(
        children: [
          20.height,
          if ((otherSettingStore.googleLoginEnable.getBoolInt() || otherSettingStore.otpLoginEnable.getBoolInt())
          ...
          if (otherSettingStore.googleLoginEnable.getBoolInt()) 16.height,
